...
db.collection("User").orderBy("time", "desc").orderBy("one").endBefore(5).limit(10).get().then(function(documentSnapshots) {
    documentSnapshots.forEach(function(doc) {
        makeTable(doc);
    });
    start_data = documentSnapshots.docs[documentSnapshots.docs.length-1];
});
    

document.getElementById("moreData").addEventListener("click", function(){
    db.collection("User").orderBy("time", "desc").orderBy("one").endBefore(5).startAfter(start_data).limit(5).get().then(function(querySnapshot) {
        querySnapshot.forEach(function(doc) {
            makeTable(doc);
        });
        start_data = querySnapshot.docs[querySnapshot.docs.length-1];
    })
...
document.getElementById("searchBtn").addEventListener("click", function(){
    ...
    if(searchDate === "" && searchName !== ""){ //이름으로만 검색
        ...
        if(getArr[i][1] != null) {
            db.collection("User").orderBy("name", "desc").where("name", ">=", searchName).limit(3).get().then(function(querySnapshot) {
                querySnapshot.forEach(function(doc) {
                    makeTable(doc);
                });
                start_data = querySnapshot.docs[querySnapshot.docs.length-1];
